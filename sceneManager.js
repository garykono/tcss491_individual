class SceneManager {
    constructor(game) {
        ...
        this.farmer = new Farmer(this.game, 2.5 * PARAMS.BLOCKWIDTH, 0 * PARAMS.BLOCKWIDTH);

        this.loadLevel(startingarea, PARAMS.BLOCKWIDTH, 0 * PARAMS.BLOCKWIDTH);
    };
    ...
    loadLevel(level, x, y) {
        ...
        if (level.grassblocks) {
            for (var i = 0; i < level.grassblocks.length; i++) {
                let grass = level.grassblocks[i];
                this.game.addEntity(new GrassBlock(this.game, grass.x, grass.y, grass.size * PARAMS.BLOCKWIDTH));
            }
        }

        if (level.wheatTiles) {
            for (var i = 0; i < level.wheatTiles.length; i++) {
                let wheat = level.wheatTiles[i];
                this.game.addEntity(new Wheat(this.game, wheat.x, wheat.y));
            }
        }

        this.game.addEntity(new Chicken(this.game, 200, this.y - 18, [{x: 400, y: 832}, {x: 100, y: 832}]));
        this.game.addEntity(new Chicken(this.game, 600, this.y - 18, [{x: 300, y: this.y - 18}, {x: 400, y: 832}, {x: 200, y: 832}, {x: 600, y: 832}]));
        // if (level.hills) {
        ...
        //     }
        // }
        this.farmer.x = this.x;
        this.farmer.y = this.y - this.farmer.height;
        this.game.addEntity(this.farmer);
    };
